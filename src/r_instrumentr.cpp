...
SEXP r_instrumentr_trace_application_load(SEXP r_context, SEXP r_application) {
    ...
    if (context->has_application_load_callback()) {
        result = r_instrumentr_trace_application(
...
SEXP r_instrumentr_trace_application_unload(SEXP r_context,
                                            SEXP r_application) {
    ContextSPtr context = Context::from_sexp(r_context);
    SEXP result = R_NilValue;
...
SEXP r_instrumentr_trace_application_attach(SEXP r_context,
                                            SEXP r_application) {
    ContextSPtr context = Context::from_sexp(r_context);
    SEXP result = R_NilValue;
...
SEXP r_instrumentr_trace_application_detach(SEXP r_context,
                                            SEXP r_application) {
    ContextSPtr context = Context::from_sexp(r_context);
    SEXP result = R_NilValue;
...
SEXP r_instrumentr_trace_package_load(SEXP r_context,
    ...
    if (context->has_package_load_callback()) {
        result =
            r_instrumentr_trace_package(ExecutionContext::PackageLoadCallback,
...
SEXP r_instrumentr_trace_package_unload(SEXP r_context,
    ...
    if (context->has_package_unload_callback()) {
        result = r_instrumentr_trace_package(
            ExecutionContext::PackageUnloadCallback,
...
SEXP r_instrumentr_trace_package_attach(SEXP r_context,
    ...
    if (context->has_package_attach_callback()) {
        result = r_instrumentr_trace_package(
            ExecutionContext::PackageAttachCallback,
...
SEXP r_instrumentr_trace_package_detach(SEXP r_context,
    ...
    if (context->has_package_detach_callback()) {
        result = r_instrumentr_trace_package(
            ExecutionContext::PackageDetachCallback,
...
SEXP r_instrumentr_trace_function_attach(SEXP r_context,
    ...
    if (context->has_function_attach_callback()) {
        result = r_instrumentr_trace_function(
            ExecutionContext::FunctionAttachCallback,
...
SEXP r_instrumentr_trace_function_detach(SEXP r_context,
    ...
    if (context->has_function_detach_callback()) {
        result = r_instrumentr_trace_function(
            ExecutionContext::FunctionDetachCallback,
...
SEXP r_instrumentr_trace_call_entry_conditionally(SEXP r_context,
    ...
    if (instrumentr::is_tracing_enabled()) {
        /* NOTE: this is here to ensure that functions called by
         * r_call_create_call do not get traced */
        instrumentr::disable_tracing();

        SEXP r_call = r_call_create_call(
            r_function, r_call_expression, r_call_environment);

        instrumentr::reinstate_tracing();

        PROTECT(r_call);

        result = r_instrumentr_trace_call_entry(
...
SEXP r_instrumentr_trace_call_exit_conditionally(SEXP r_context,
    ...
    if (instrumentr::is_tracing_enabled()) {
        result = r_instrumentr_trace_call_exit(
            r_context, r_application, r_package, r_function, r_result);
    }
