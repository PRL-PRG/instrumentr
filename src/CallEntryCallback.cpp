...
SEXP CallEntryCallback::class_ = nullptr;

void CallEntryCallback::initialize() {
    class_ = Callback::create_class("instrumentr_call_entry_callback");
    R_PreserveObject(class_);
}

void CallEntryCallback::finalize() {
    R_ReleaseObject(class_);
    class_ = NULL;
}

SEXP CallEntryCallback::get_class() {
    return class_;
}

void CallEntryCallback::invoke(SEXP r_context,
                               SEXP r_application,
                               SEXP r_package,
                               SEXP r_function,
                               SEXP r_call) {
